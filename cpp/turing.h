...
#pragma endregion
  ...
  std::vector<std::pair<cell_t, std::vector<Act>>> parceStrToTable(
      std::string str) {
    auto csv_input = readCSV(str);

    for (auto& i : csv_input)
      for (auto& j : i) j = removeSpace(j);
  ...
  bool iterate() {
    ...
    if (search_res != optimized_table.end()) {
      if (state >= search_res->second.size() || state < 0)
        throw(std::runtime_error("Current state is outside of table"));
    ...
    if (act.ch == ERR) {
      std::string er = "Looks like you have some error in algorithm " +
                       std::string(search_res->first);
      ...
      do {
        if (show == -1 ||
            std::chrono::duration<double, std::milli>(
                std::chrono::high_resolution_clock::now() - last_show)
                    .count() > show) {
  ...
  TuringMachine(std::string s,
                std::vector<std::pair<cell_t, std::vector<Act>>> ntable,
                int pos = 0) {
    setDefault(s, pos);
    table = ntable;
  }
